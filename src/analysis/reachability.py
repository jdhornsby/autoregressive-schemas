...
Walks `results/judgments/reachability_<cond>_<suffix>_*.json` and computes the
fraction of encounters where the inventory at that point fails to satisfy the
enemy's weakness. Outputs a wide CSV (variant × condition) and a grouped bar
chart matching the style of `04_thinking_chart.png`.
"""
...
def aggregate() -> pd.DataFrame:
    """Return a long-form DataFrame: variant, condition, files, encounters, breaks, rate_pct."""
    rows: list[dict] = []
    jdir = judgments_dir()
...
def run() -> dict:
    ...
    wide_for_csv = wide.rename(columns=CONDITION_LABELS)
    wide_for_csv.to_csv(wide_path, float_format="%.1f")
